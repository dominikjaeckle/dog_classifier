...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print("Classification based on: {used_model}".format(used_model=model))
    
    print("\n# images: {n_images}".format(n_images=results_stats_dic['n_images']))
    print("# dog images: {n_dogs_img}".format(
        n_dogs_img=results_stats_dic['n_dogs_img']))
    print("# non-dog images: {n_notdogs_img}".format(
        n_notdogs_img=results_stats_dic['n_notdogs_img']))

    print("\n% correct dogs: {pct_correct_dogs}".format(
        pct_correct_dogs=results_stats_dic['pct_correct_dogs']))
    print("% correct breed: {pct_correct_breed}".format(
        pct_correct_breed=results_stats_dic['pct_correct_breed']))
    print("% correct not-a dog: {pct_correct_notdogs}".format(
        pct_correct_notdogs=results_stats_dic['pct_correct_notdogs']))
    print("% match: {pct_match}".format(
    ...
    if (print_incorrect_dogs == True) and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']):
            ...
            for key in results_dic:
                if sum(results_dic[key][3:]) == 1:
                    print("-- image: {image}  |  classifier label: {label}".format(
                        image=key, label=results_dic[key][1]))
